...
export function KosDetailPage({ kos, onBack }: KosDetailPageProps) {
  ...
  useEffect(() => {
    let isCurrent = true
    getPaymentQuote(kos.id, rentalMonths, paymentChoice)
      .then((quote) => {
        if (isCurrent) setServerQuote(quote)
      })
      .catch(() => {
        if (isCurrent) setServerQuote(null)
      })
    return () => {
      isCurrent = false
    }
  }, [kos.id, paymentChoice, rentalMonths])

  function requireRenterLogin() {
    if (!user) {
      navigate('/login/pencari-kos')
      return false
    }
    if (user.role !== 'pencari-kos') {
      setActionStatus('Masuk sebagai pencari kos untuk menggunakan fitur ini.')
      return false
    }
    return true
  }
  ...
  const displayedBreakdown =
    activeBreakdown && serverQuote && breakdownType === serverQuote.paymentMethod
      ? {
          ...activeBreakdown,
          lines: serverQuote.lineItems.map((line) => ({
            label: line.label,
  ...
  return (
    <section className="mx-auto w-full min-w-0 max-w-7xl overflow-x-clip px-4 pb-16 pt-6 sm:px-8 lg:px-[86px]">
      ...
      {displayedBreakdown && (
        <PaymentBreakdownModal
          title={displayedBreakdown.title}
          lines={displayedBreakdown.lines}
          totalLabel={displayedBreakdown.totalLabel}
          total={displayedBreakdown.total}
          isAuthoritative={Boolean(serverQuote && breakdownType === serverQuote.paymentMethod)}
